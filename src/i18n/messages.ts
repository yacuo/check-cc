...
export const messages: Record<LocaleCode, Messages> = {
  zh: {
    ...
    faq: { title: "Claude 环境检测 FAQ", items: zhFaqs },
  ...
  "zh-HK": {
    ...
    signals: { eyebrow: "Risk Signals", title: "我們會檢測哪些 Claude 風險信號？", desc: "這些檢測項直接渲染在靜態 HTML 中；實際檢測時會結合瀏覽器端與服務端信號動態評分。", checks: [["Claude 同款時區", "讀取系統時區與 UTC 偏移，捕捉地區特徵。"], ["語言與 Intl 指紋", "navigator.languages、Accept-Language、Intl locale 聯合識別。"], ["字體側信道探測", "Canvas 寬度探測中文字體與廠商字體。"], ["瀏覽器 / WebView", "匹配微信、QQ、夸克、UC、百度等環境線索。"], ["設備品牌畫像", "識別鴻蒙、華為、小米、OPPO、vivo 等弱信號。"], ["網絡出口校驗", "IP 地區、邊緣請求頭、節點出口與環境一致性分析。"]] },
    regionsTitle: "重點地區檢測",
  ...
  ru: {
    ...
    signals: { eyebrow: "Risk Signals", title: "Какие сигналы риска Claude проверяются?", desc: "Эти проверки отображаются в HTML и затем уточняются динамически по сигналам браузера и сервера.", checks: [["Часовой пояс Claude", "Чтение системного часового пояса и UTC-смещения."], ["Язык и Intl", "Совместная проверка navigator.languages, Accept-Language и Intl locale."], ["Проверка шрифтов", "Canvas-измерение ширины шрифтов для региональных признаков."], ["Браузер / WebView", "Определение окружений WeChat, QQ, Quark, UC, Baidu."], ["Профиль устройства", "Слабые сигналы HarmonyOS, Huawei, Xiaomi, OPPO, vivo."], ["Сетевой выход", "Анализ IP-региона, edge-заголовков и согласованности сети."]] },
